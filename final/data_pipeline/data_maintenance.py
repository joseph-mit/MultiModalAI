...
# - downloads listing photos from Zillow URLs
# - get property details by scraping listing descriptions + price history
...
PHOTOS_DIR = BASE / 'listing_photos'
...
def fill_missing_census(csv_path=MASTER_CSV):
    ...
    census_cols = ['census_median_income', 'census_median_home_value', 'census_median_rent',
                   'census_pct_educated', 'census_total_population', 'census_pct_white',
                   'census_pct_black', 'census_pct_asian', 'census_pct_hispanic']
    ...
    for idx, row in missing.iterrows():
        ...
        filled_count += 1
        print(f"[{filled_count} / {len(missing)}]")
...
def download_photos_for_row(row, max_workers=8):
    ...
    tasks = [
        (url, prop_dir / f"{prop_id}_listing_{i}.jpg")
...
if __name__ == '__main__':
    print("1. Getting missing census data:")
    fill_missing_census()
